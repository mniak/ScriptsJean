...
func TestDirNamesForSingleDate(t *testing.T) {
	t.Run("Example from video", func(t *testing.T) {
		...
		assert.Equal(t, "23 07 09 Sonn. Fotos/23 07 09 Ecobie/Ecobie 4", dirnames[5])
	})
	...
	t.Run("Generated example", func(t *testing.T) {
		...
		assert.Equal(t, "07 06 05 Diens. Fotos/07 06 05 Ecobie/Ecobie 4", dirnames[5])
	})
}
